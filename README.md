...
runs it only once you have read it. It resolves in three ways: the reader's
own API key saved in their browser, the site's `api/ask.js` endpoint when
`ANTHROPIC_API_KEY` is configured, or built-in question patterns that work
...
Full detail: [`docs/querying.md`](docs/querying.md).
...
[`docs/population.md`](docs/population.md) runs population end to end: fetch
WorldPop, summarise it onto districts, and answer the questions that follow —
who is far from a clinic, who lives on ground that has flooded, which
districts run hot and how many people are in them.
...
`05_zonal_stats.py` is the engine underneath: it summarises any raster onto
any boundary set, so population, surface temperature, built-up area, tree
cover and rainfall are all the same operation with different inputs. `--zones`
takes any vector file, so rainfall can be reported per catchment rather than
per district.
